...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('.nav-tab[data-view]').forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            if (view === 'param-search') { const sel = document.getElementById('param-select'); if (sel.options.length <= 1) loadParamSelect(); }
        });
    });
    // Buttons
...
async function handleLogout() { try { await apiFetch('/api/auth/logout', { method: 'POST', headers: getCsrfHeaders() }); localStorage.removeItem('djezzy_server'); window.location.reload() } catch (e) { } }
...
async function loadTopology() {
    ...
    try {
        ...
        const r = await apiFetch(url); const d = await r.json(); _topoData = d;
        if (!d.nodes?.length) { container.innerHTML = '<div class="empty-state"><p>No topology data</p></div>'; return; }
        ...
        _topoAllNodes = new vis.DataSet(d.nodes.map(n => {
            ...
            return {
                ...
                color: { background: isCenter ? '#E11326' : hColor, border: isCenter ? '#FFF' : '#333', highlight: { background: '#FFF', border: hColor } },
        ...
        _topoAllEdges = new vis.DataSet(d.edges.map((e, i) => ({
            ...
            width: e.status === 'ok' ? 1 : 2.5, dashes: e.status === 'fault',
        ...
        _topoNetwork = new vis.Network(container, { nodes: _topoAllNodes, edges: _topoAllEdges }, {
            ...
            interaction: { hover: true, tooltipDelay: 200 }, nodes: { borderWidth: 2 }, edges: { smooth: { type: 'continuous' } }
        });
        const info = document.getElementById('topo-info');
        _topoNetwork.on('click', params => {
            if (params.nodes.length) {
                const nid = params.nodes[0]; const nd = d.nodes.find(n => n.id === nid);
                const conns = d.edges.filter(e => e.from === nid || e.to === nid);
                const okCount = conns.filter(e => e.status === 'ok').length; const faultCount = conns.length - okCount;
                const hBadge = nd.health === 'healthy' ? '🟢' : (nd.health === 'warning' ? '🟡' : '🔴');
                let faultList = '';
                if (faultCount > 0) {
                    const faultEdges = conns.filter(e => e.status === 'fault');
                    faultList = '<div style="margin-top:.4rem;font-size:.75rem"><strong>Faulty:</strong><ul style="margin:.2rem 0;padding-left:1rem">' +
                        faultEdges.map(e => { const peer = e.from === nid ? e.to : e.from; return `<li>${peer} — X2: ${e.x2_status}, SCTP: ${e.sctp_status}</li>`; }).join('') + '</ul></div>';
                }
                info.style.display = 'block';
                info.innerHTML = `<div style="display:flex;align-items:center;gap:.4rem;flex-wrap:wrap"><span style="font-size:1rem">${hBadge} <strong>${nid}</strong></span><span style="font-size:.75rem;color:#999">${nd.tech}</span><span style="margin-left:auto;font-size:.8rem">✅ ${okCount} | ❌ ${faultCount}</span></div>${faultList}`;
            } else { info.style.display = 'none'; }
        });
        const cb = document.getElementById('topo-faults-only'); if (cb) cb.checked = false;
        showToast(`${d.nodes.length} sites, ${d.edges.length} links`, 'success');
...
async function uploadDataset() { const key = document.getElementById('upload-dataset-select').value; const fi = document.getElementById('upload-file-input'); if (!key) return showToast('Select a category', 'error'); if (!fi.files?.length) return showToast('Select file(s)', 'error'); const fd = new FormData(); for (let i = 0; i < fi.files.length; i++) fd.append('file', fi.files[i]); fd.append('dataset', key); showToast(`Uploading ${fi.files.length} file(s)...`, 'info'); try { const r = await apiFetch('/api/admin/upload', { method: 'POST', headers: { 'X-CSRFToken': csrfToken }, body: fd }); const d = await r.json(); showToast(d.message || d.error, r.ok ? 'success' : 'error'); if (r.ok) { fi.value = ''; loadDashboard(); } } catch (e) { showToast('Upload error', 'error'); } }
